...
class FixedConstraint {
    ...
    // mass, vel, pos, moi, rot_vel, angle
    composite_body = {
        mass: 0,
        vel: new Vec2D(0, 0),
        pos: new Vec2D(0, 0),
    ...
    calc_utils() {
        ...
        : this.A.mass == Infinity ? 0 
        : this.B.mass / total_mass;
        const a_dist = this.distance * theta;
        const b_dist = this.distance * (1 - theta);
        const total_moi = this.A.moi + this.A.mass * a_dist * a_dist + this.B.moi + this.B.mass * b_dist * b_dist;
        const rel_angle = Math.atan2(this.B.pos.y - this.A.pos.y, this.B.pos.x - this.A.pos.x);
        const dir = Vec2D.normalize(new Vec2D(Math.cos(rel_angle), Math.sin(rel_angle)));
        const perp = new Vec2D(-dir.y, dir.x);
        
        this.utils.total_mass = total_mass;
        this.utils.total_moi = total_moi;
        this.utils.theta = theta;
        this.utils.a_dist = a_dist;
        this.utils.b_dist = b_dist;
        this.utils.rel_angle = rel_angle;
        this.utils.dir = dir;
        this.utils.perp = perp;
    }

    update_composite_body() {
        this.calc_utils();
        const {total_mass, total_moi, theta, rel_angle, perp} = this.utils;

        this.composite_body.pos = Vec2D.mult(this.B.pos, theta).add(Vec2D.mult(this.A.pos, 1 - theta));
        this.composite_body.vel = Vec2D.mult(this.B.vel, theta).add(Vec2D.mult(this.A.vel, 1 - theta));
        this.composite_body.mass = total_mass; 
        this.composite_body.angle = rel_angle;
        this.composite_body.rot_vel = Vec2D.sub(this.A.vel, this.B.vel).dot(perp) / this.distance;
        this.composite_body.moi = total_moi;
    }
    ...
    apply_composite_body() {
        ...
        this.A.vel = Vec2D.mult(perp, -rot_vel * a_dist).add(vel);
        this.B.vel = Vec2D.mult(perp, rot_vel * b_dist).add(vel);

        this.A.angle = angle + this.a_target_angle;
        this.B.angle = angle + this.b_target_angle;
